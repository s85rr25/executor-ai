...
### Phoenix first (~30 min, `agent/observability/phoenix.py`)
Stand this up before anything else. Initialize Phoenix / OpenInference so Anthropic and
OpenAI calls are auto-instrumented, and expose a small span helper the
...
| **Everyone** | Phoenix wired up so all Claude and OpenAI calls are traced |
...
- **Phoenix** — your Phoenix setup traces every call across the service. In the demo, show
