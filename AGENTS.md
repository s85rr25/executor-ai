...
- **Documents**: `pypdf` / `pdfplumber` for text, Claude vision for scans/images;
  `pillow` + `pillow-heif` for images/HEIC
...
- **State + vectors**: KV holds estate state; a vector store powers RAG / agent memory
  ...
  supports three backends via `STORE_BACKEND`: `redis_cloud` (Redis Cloud KV + Redis 8
  Vector Sets — the cloud path this project runs on), `upstash` (Upstash Redis REST +
  Upstash Vector — also supported), and `memory` (in-process fallback).
...
| ResearchAgent (probate-law watch) | `agent/researcher/research_agent.py` |
...
See `agent/.env.example` and `web/.env.local.example` for the authoritative lists.
...
Appointment: 2026-06-10 | Executor: Dana Milligan. Canonical object:
...
2. **CRITICAL** — DE-160 Inventory & Appraisal outstanding, no appraisal on the Berkeley
   home or the Honda Civic.
