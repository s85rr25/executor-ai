...
type Doc = { id: string; name: string; type: string; docType: string; parsed: boolean };

function documentTypeLabel(documentType: string) {
  return documentType.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase());
}

// Maps a parsed document's type onto the "What to upload" checklist item id.
const DOC_TYPE_TO_CHECKLIST: Record<string, string> = {
  will: "will",
  bank_statement: "bank",
  deed: "deed",
};
...
function toDoc(d: { id: string; fileName: string; documentType: string }): Doc {
  return { id: d.id, name: d.fileName, type: documentTypeLabel(d.documentType), docType: d.documentType, parsed: true };
}
...
export function UploadScreen({ estate }: Props) {
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      // Re-read the estate so docs carry their real backend ids (needed to fetch
      // the stored file for preview/download) and assets reflect the new parse.
      const refreshed = await getEstate(estateId);
      setAssets(refreshed.assets.map(fromEstateAsset));
      setDocs(refreshed.documents.map(toDoc));
    } catch (error) {
      console.error(error);
  ...
  const uploadedChecklistIds = new Set(
    docs.map((d) => DOC_TYPE_TO_CHECKLIST[d.docType]).filter(Boolean) as string[],
  );
      ...
      <Card title="What to upload" subtitle="Required documents first, then optional ones you only need if they apply to this estate" padded={false}>
        <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
          {(DOC_CHECKLIST || []).map((d, i) => {
            // Uploaded if the demo seeded it, or a real parsed doc maps to it.
            const up = (seeded && d.uploaded) || uploadedChecklistIds.has(d.id);
          ...
          <div role="dialog" aria-modal="true" aria-label={openDoc.name} onClick={(e) => e.stopPropagation()}
            ...
            <div style={{ padding: "var(--space-6)", background: "var(--surface-sunken)" }}>
              {seeded ? (
                ...
                <iframe title={openDoc.name} src={fileUrl(openDoc)} style={{ width: "100%", height: "60vh", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-sm)", background: "var(--paper-0)" }} />
              )}
            </div>
