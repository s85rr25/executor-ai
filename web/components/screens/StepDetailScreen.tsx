  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "28px 40px 48px" }}>
      ...
      <section style={{ marginTop: "var(--space-8)" }}>
        ...
        <p style={{ margin: 0, fontSize: "var(--text-md)", lineHeight: "var(--leading-relaxed)", color: "var(--text-body)" }}>{cleanDashboardText(item.body)}</p>
        {item.rule ? (
          <p style={{ margin: "var(--space-3) 0 0", fontFamily: "var(--font-mono)", fontSize: "var(--text-xs)", color: "var(--text-muted)" }}>{cleanDashboardText(item.rule)}</p>
        ) : null}
      </section>
        ...
        <ol style={{ margin: 0, padding: 0, listStyle: "none", display: "grid", gap: "var(--space-4)" }}>
          {item.steps && item.steps.map((s, i) => (
            <li key={i} style={{ display: "flex", gap: "var(--space-4)", alignItems: "flex-start" }}>
              ...
              <p style={{ margin: 0, paddingTop: 3, fontSize: "var(--text-base)", lineHeight: "var(--leading-relaxed)", color: "var(--text-body)" }}>{cleanDashboardText(s)}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      >
        We'll move "{cleanDashboardText(item.title)}" off your attention list and into completed steps. You can reopen it any time, nothing is deleted.
      </Dialog>
    </div>
  );
}
